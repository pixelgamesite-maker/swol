...
const DRAFT_KEY = "swol_wl_draft";import { useState, useEffect } from "react";
import { supabase } from "../lib/supabase";
import { colors, display, mono, sans } from "../lib/theme";
import {
  X_URL, PINNED_TWEET_URL, isValidEvm, isValidXUrl,
...
export default function WhitelistApplication({ open, onClose, communityName }: WhitelistApplicationProps) {
  ...
  /* ── Persist draft ── */
  useEffect(() => {
    if (ready) localStorage.setItem(DRAFT_KEY, JSON.stringify({ twitter, wallet, quoteUrl, commentUrl, followed, step }));
  }, [twitter, wallet, quoteUrl, commentUrl, followed, step, ready]);

  const valid = [
    twitter.trim().length > 1,
    followed,
    isValidXUrl(quoteUrl),
    isValidXUrl(commentUrl),
    isValidEvm(wallet),
  ];
            ...
            <div key={step} style={{ animation:"stepIn 0.25s ease both", minHeight:"170px" }}>
              ...
              {step === 3 && (
                <div>
                  ...
                  <p style={{ fontFamily:sans, fontSize:"0.85rem", color:colors.textDim, margin:"0 0 14px", lineHeight:1.55 }}>
                    Comment on the pinned post and tag 2 Swoldiers. Paste your comment link below.
                  </p>
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  {commentUrl && !isValidXUrl(commentUrl) && <p style={{ fontFamily:sans, fontSize:"0.6rem", color:colors.danger, margin:"6px 0 0" }}>Needs a valid https://x.com/.../status/... link</p>}
                  {!openedComment && !commentUrl && <p style={{ fontFamily:sans, fontSize:"0.6rem", color:"rgba(255,255,255,0.28)", margin:"8px 0 0" }}>Open the comment popup above, post it, then paste your link here.</p>}
                </div>
              )}
